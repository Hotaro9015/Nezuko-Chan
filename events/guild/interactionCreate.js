		...
		try {
			...
			if (!interaction.guild.me.permissions.has(Discord.Permissions.FLAGS.USE_EXTERNAL_EMOJIS))
				return interaction.reply({
					content: `<:no:995153594032599112> I am missing the Permission to \`USE_EXTERNAL_EMOJIS\``,
					ephemeral: true
				})
			...
			if (interaction.isCommand()) {
                    ...
                    if (command.premium && premiumServer && !premiumServer.isPremium) {
                        return interaction.reply({
                            embeds: [new MessageEmbed()
                                ...
                                Join **[Nezuko](${config.env.SUPPORT})** And **Open a Ticket!**`)
                                .setColor("#FFFF00")
                            ],
                            ephemeral: true
                        })
                ...
                commandLogsChannel.send({
                    embeds: [new MessageEmbed()
                        ...
                        .setTitle(`<:arrow:995142422684237834> Slash Command`)
                        .addField("**<:arrow:995142422684237834> Author**", `\`\`\`${interaction.user.tag}\`\`\``)
                        .addField("**<:arrow:995142422684237834> Command Name**", `\`\`\`${command.name}\`\`\``)
                    ]
                });
				}
			}
